...
<LINK href="admin/css/admin.css" type="text/css" rel="stylesheet">
  ...
  <TR height=22>
    ...
    align=middle background=admin/images/title_bg2.jpg>修改诊断</TD></TR>
  ...
  <TR height=20>
    ...
    <TABLE cellSpacing=1 cellPadding=2 width="70%" align=center border=0 bgcolor="#cccccc">
    	...
    	<tr bgcolor="#ffffff" align="center">
    		<td>病情：</td>
    		<td colspan="2">
    		<textarea rows="5" cols="100" id="bq" name="bq" ><%=map.get("bq") %></textarea>
    		</td>
    	</tr>
    	<tr bgcolor="#ffffff" align="center">
    		<td  align="center">诊断：</td>
    		<td colspan="2">
    		<textarea rows="5" cols="100" id="zd" name="zd" ><%=map.get("zd") %></textarea>
    		</td>
    	</tr>
    	<tr bgcolor="#ffffff">
    		...
    		<input id="Hidden2" name="Hidden2" type="hidden" value="<%=numinfo %>" />
    	</tr>
    	<tr bgcolor="#ffffff">
...
if(suc!=null)
{
 %>
 alert("操作成功");
location.replace("/srykzsblmis/admin/jzghrecord.jsp");
 <%}%>
</script>
...
var num = 0;
  ...
  if(!info.equals("")&&info!=""&&info!=null){%>
	...
	var numinfoArray = numinfo.split("\\$");
	...
	row = document.getElementById(id).insertRow();
		...
		cell.innerHTML=""+(i+1)+" <input  onfocus='loadXMLDoc(this)' value='"+infoArray[i]+"'  id='St"+i+"' type='text' name='St"+i+"' size='50'> ;数量:<input type='text' value='"+numinfoArray[i]+"' id='num"+i+"' name='num"+i+"'  />";
